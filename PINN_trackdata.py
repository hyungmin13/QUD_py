...
if __name__ == "__main__":
    ...
    timeskip = 1
    track_limit = 1000000000
    frequency = 3000
    domain_range = {'t':(0,10/frequency), 'x':(0,1.2), 'y':(0,1.2), 'z':(0,1.0)}
    viscosity = 15*10**(-6)
    ...
    all_params["data"] = Data.init_params(path = path, domain_range = domain_range, 
                                          ...
                                          viscosity = viscosity, data_split = data_split)
    
    train_data, valid_data, all_params = Data.train_data(all_params)
...
# %%
    print(np.max(train_data['pos'][:,1]), np.min(train_data['pos'][:,1]))
    print(np.max(train_data['pos'][:,2]), np.min(train_data['pos'][:,2]))
    print(np.max(train_data['pos'][:,3]), np.min(train_data['pos'][:,3]))
